...
| --- | --- | --- |
| Repository foundation and modular monolith | Implemented, governance partial | Project requirements, module boundaries, architecture tests, engineering workflows, and ADR-0001 are present. ADR-0002 through ADR-0010 remain Proposed and must not be treated as accepted production decisions. |
...
| Core domain modules | Implemented vertical slices, partial product coverage | Organizations, People, Entitlements, Academics, Admissions, Grading, Scheduling, Audit, Outbox, Provisioning, Notifications, Integrations, self-service reads, resumable accepted-applicant conversion, explicit academic-enrollment and subscription lifecycle transitions, and Moodle grade-evidence acceptance have real application and persistence paths. Deferred product areas remain explicitly unavailable, and the complete critical journey still requires controlled end-to-end and institutional-policy evidence. |
...
| Moodle | Partial and credential-gated | Tenant configuration, encrypted tokens, mappings, supported REST calls, provisioning seams, duplicate-safe grade evidence/status, mapped-user deadline reads, tenant-signed grade-event ingress, selected-term reconciliation runs, reviewer-visible evidence state, and explicit Grading acceptance or rejection of evidence exist (ADR-0010, Proposed). Deadline results are bounded live evidence from the mapped user's enrolled courses, with observation time and source version; there is no persisted deadline cache. Mapping management has no administrative workflow, and real Moodle version, permission, outage, throttling, reconciliation-function, and event-sender exercises require a controlled instance. |
...
| Frontend experience | Partial | The Next.js shell, organization context, role/permission/entitlement navigation, connected self-service reads, explicit unavailable states, and interactive administrative workflows for organization structure, curricula, admissions, people, grading scales, branding, provisioning retries, Moodle configuration and grade-evidence review, and platform plans, features, entitlements, and subscription lifecycle exist, each with component tests over stubbed authorized requests. Accessibility, localization, browser compatibility, and browser-driven end-to-end coverage remain incomplete. |
