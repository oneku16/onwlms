...
These notes are for cloud agents running in an environment where the startup update script (backend `uv sync`, frontend/openapi-contract `npm ci`) has already run and the system toolchain is present in the VM snapshot. They record only non-obvious, durable startup/run caveats. Standard commands live in `README.md` and the `Makefile` (`make setup|run|test|lint|typecheck|migrate|seed`); use those rather than duplicating them.

- Toolchain is pre-installed in the snapshot: `uv` (with Python 3.14), Node.js 24 + npm 11, and Docker Engine 29 with Compose v2. The infra base `PATH` contains an older `/exec-daemon/node` (v22); working `node`/`npm`/`uv` symlinks are placed in `/usr/local/cargo/bin` (earlier in `PATH`) so the correct versions win in all shells. If `node -v` ever reports v22, that shadowing is why — use the `/usr/local/cargo/bin` entries.
- The Docker daemon does NOT auto-start on VM boot (the update script must not start services). Start it once per session before any `docker`/`docker compose`/`make run`/testcontainers use: `sudo dockerd > /tmp/dockerd.log 2>&1 &` (or run it in a tmux session), then wait until `docker info` succeeds. Fresh login shells already have the `docker` group, so `docker` works without `sudo`; a shell started before the group was applied may need `sg docker -c '...'` or `newgrp docker`.
- `.env` is git-ignored and required to run the stack. `make setup` (or `scripts/setup.sh`) creates it from `.env.example` and generates the three Fernet keys (`SESSION_/PII_/INTEGRATION_ENCRYPTION_KEY`); it is idempotent and never overwrites existing values. `make setup` also runs `docker compose build`, so ensure the Docker daemon is running first.
- Local development identity: there is no dev-login button in the UI (the sign-in page only offers real OwnID, which needs external credentials). With `DEV_AUTH_ENABLED=true` (set in `.env.example` and compose), authenticate via `POST /api/v1/auth/dev-login`, which establishes the session as the seeded platform admin. State-changing requests use double-submit CSRF: echo the readable `ownsis_csrf` cookie in the `X-CSRF-Token` header (the frontend client does this automatically once the cookie is set). Run `make seed` after migrations to create the local platform admin, demo tenant, and base plan.
- PostgreSQL tenant-enforcement tests are opt-in and connect to `localhost:5432` (they do NOT spin up their own container). Bring Postgres up and migrate first (`docker compose up -d --wait postgres` then `make migrate`, or a full `make run`), then run with `OWNSIS_RUN_POSTGRES_INTEGRATION=1`. Without it those ~9 tests are skipped, not failed.
- Service ports (bound to 127.0.0.1): frontend `3000`, backend API `8000` (`/health`, `/ready`, `/api/docs`), PostgreSQL `5432`.
